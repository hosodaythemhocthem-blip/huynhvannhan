  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setExams(ex || []);
        setSubmissions((sub || []).filter(s => s.student_id === user.id));
      } catch (e) {
      ...
      >
        ...
        <div className="relative z-10 space-y-8">
          <div className="inline-flex items-center gap-3 px-5 py-2 bg-white/10 backdrop-blur-xl rounded-full border border-white/20">
            ...
            <span className="text-[11px] font-black uppercase tracking-[0.2em]">Hệ sinh thái NhanLMS v5.9</span>
          </div>
...
const ExamCard = ({ exam, sub }: any) => (
  <MotionDiv whileHover={{ y: -8 }} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all group flex flex-col justify-between h-full">
    ...
    <div className="pt-6 border-t border-slate-50 flex items-center justify-between">
      ...
      <button className={`px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${sub ? 'bg-slate-100 text-slate-400' : 'bg-indigo-600 text-white shadow-lg hover:bg-black'}`}>
        {sub ? 'XEM LẠI' : 'BẮT ĐẦU'}
      </button>
    </div>
  </MotionDiv>
